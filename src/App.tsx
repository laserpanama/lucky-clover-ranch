...
export default function App() {
  ...
  // Compute availability for filtering
  const getAnimalAvail = (animalId: number) => getAnimalAvailability(animalId, rentals);
  const availableCount = animals.filter(a => getAnimalAvail(a.id).isAvailable).length;
  const rentedCount = animals.length - availableCount;
  
  const filteredAnimals = animals
    .filter(a => categoryFilter === 'all' || a.category === categoryFilter)
    .filter(a => {
      if (availabilityFilter === 'available') return getAnimalAvail(a.id).isAvailable;
      if (availabilityFilter === 'rented') return !getAnimalAvail(a.id).isAvailable;
      return true;
    })
    ...
    <div className="min-h-screen bg-[#F4F6F8] text-[#1A1A1A] font-sans">
      <aside className="fixed left-0 top-0 h-full w-64 bg-white border-r border-slate-200 p-6 z-10 flex flex-col">
        ...
        <nav className="space-y-1">
          {navItems.map(item => (
            <button key={item.id} onClick={() => { setActiveTab(item.id); setSearch(''); setCategoryFilter('all'); setAvailabilityFilter('all'); }}
              ...
              {activeTab === 'animals' && (
                <div>
                  ...
                  <div className="flex items-center gap-2 mb-6">
                    <button onClick={() => setAvailabilityFilter('all')} className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${availabilityFilter === 'all' ? 'bg-slate-700 text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}>All Status</button>
                    <button onClick={() => setAvailabilityFilter('available')} className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all flex items-center gap-1.5 ${availabilityFilter === 'available' ? 'bg-emerald-600 text-white' : 'bg-emerald-50 text-emerald-600 hover:bg-emerald-100'}`}><CheckCircle2 className="w-3 h-3" />Available ({availableCount})</button>
                    <button onClick={() => setAvailabilityFilter('rented')} className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all flex items-center gap-1.5 ${availabilityFilter === 'rented' ? 'bg-rose-600 text-white' : 'bg-rose-50 text-rose-600 hover:bg-rose-100'}`}><AlertCircle className="w-3 h-3" />Rented ({rentedCount})</button>
                  </div>
